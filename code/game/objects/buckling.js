...
class Buckle extends Component {
	...
	buckle_mob(buckling, force = false, check_loc = true) {
		...
		if((!this.can_buckle && !force) || buckling.buckled || (this.buckled_mobs.length >= this.max_buckled_mobs) || this.atom == buckling) { //TODO: restraints check
			return false;
		}
	...
	user_buckle_mob(buckling, user, check_loc = true) { //This proc is unfinished placeholder, using it for testing.
		...
		this.buckle_mob(buckling, false, check_loc);
		if(buckling == user) {
...
Buckle.depends = ["LivingMob", "MobHud"];
